...
function CalculatorView({players,games,expenses,credits,balances}:{players:Player[];games:Game[];expenses:Expense[];credits:Credit[];balances:PlayerBalance[]}) {
  ...
  const rows=players.map(player=>{
    ...
    const leagueFeeShare=weightedExpenses.filter(expense=>isLeagueFee(expense.category)).reduce((sum,expense)=>sum+gameWeightedShare(expense.amount,player.id,games),0);
    const refreshmentShare=weightedExpenses.filter(expense=>!isLeagueFee(expense.category)).reduce((sum,expense)=>sum+gameWeightedShare(expense.amount,player.id,games),0);
    ...
    const umpiringCostShare=credits.filter(isUmpiringWaiver).reduce((sum,credit)=>sum+gameWeightedShare(credit.amount,player.id,games),0);
    ...
    const weight=weightedTotal?gameWeightedShare(weightedTotal,player.id,games)/weightedTotal:0;
    return {player,gamesPlayed,appearances,umpiredGames,umpiringDetails,umpiringCredit,umpiringCostShare,weight,leagueFeeShare,refreshmentShare,otherExpenseShare,creditShare,total:leagueFeeShare+refreshmentShare+otherExpenseShare+umpiringCostShare+creditShare};
...
function ExpenseModal({expense,memberPlayerId,existingExpenses,leagueFeeExists,players,games,onAddPlayer,onClose,onSave,onDelete}:{expense?:Expense;memberPlayerId?:number;existingExpenses:Expense[];leagueFeeExists:boolean;players:Player[];games:Game[];onAddPlayer?:(name:string)=>number;onClose:()=>void;onSave:(e:Omit<Expense,"id">)=>void;onDelete?:()=>void}) {
  ...
  return <div className="modal-backdrop"><form className="modal lineup-modal" onSubmit={e=>{e.preventDefault();if(canSave&&!duplicate)onSave({label:label.trim(),amount:Number(amount),category,paidBy,split,date,participants:split==="appearances"?undefined:[...participants]})}}>
    ...
    {split==="appearances"&&<div className={fundedGameCount?"split-note":"split-note warning"}><strong>{category} · {fundedGameCount} completed {fundedGameCount===1?"game":"games"}</strong><span>{fundedGameCount?"This expense is divided equally across completed games, then among each game’s eligible players. Excluded players are not charged for that game.":"Mark games Completed and include lineup players; this expense will recalculate automatically."}</span></div>}
