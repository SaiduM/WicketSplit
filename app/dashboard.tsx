...
function TeamMemberAccessModal({team,onClose,notify}:{team:Team;onClose:()=>void;notify:(message:string)=>void}){
  const [creating,setCreating]=useState(false);const [access,setAccess]=useState<{url:string;pin:string}|null>(null);
  ...
  const create=async()=>{setCreating(true);const response=await fetch("/api/team-access",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({teamId:team.id})});const result=await response.json().catch(()=>({})) as {url?:string;pin?:string;error?:string};setCreating(false);if(!response.ok||!result.url||!result.pin){notify(result.error??"Team member access could not be created");return}setAccess({url:result.url,pin:result.pin});notify("New team member link and PIN created")};
  ...
  const revoke=async()=>{if(!confirm(`Revoke shared member access for ${team.name}? Existing team-member sessions will be signed out.`))return;const response=await fetch("/api/team-access",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({teamId:team.id})});if(!response.ok){const result=await response.json().catch(()=>({})) as {error?:string};notify(result.error??"Access could not be revoked");return}setAccess(null);notify("Shared team member access revoked")};
  return <div className="modal-backdrop"><section className="modal small-modal member-access-modal"><ModalHead eyebrow="TEAM MEMBER MODE" title="Share one team link" description="Players use a private link and 6-digit PIN, select their roster name, and enter without creating an account." close={onClose}/>{access?<div className="invite-preview"><label>WhatsApp message<textarea readOnly value={message} onFocus={event=>event.currentTarget.select()}/></label><small>Creating another link immediately invalidates this link, PIN, and existing shared-member sessions.</small></div>:<div className="member-access-intro"><strong>{team.players.length} roster players can use shared access</strong><p>Members can view team records and add only expenses paid by the player they select. They cannot change rosters, leagues, games, credits, settlements, or invitations.</p><div><span>1</span>Create the private link and PIN</div><div><span>2</span>Send both in your team WhatsApp group</div><div><span>3</span>Each player chooses their own roster name</div></div>}<div className="modal-actions">{access&&<button type="button" className="danger-action" onClick={revoke}>Revoke access</button>}<button type="button" className="ghost" onClick={onClose}>Done</button>{access?<><button type="button" className="ghost" disabled={creating} onClick={create}>{creating?"Replacing…":"Replace link & PIN"}</button><button type="button" className="primary" onClick={copy}>Copy WhatsApp message</button></>:<button type="button" className="primary" disabled={creating||!team.players.length} onClick={create}>{creating?"Creating…":"Create link & PIN"}</button>}</div></section></div>;
}
