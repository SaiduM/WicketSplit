    ...
    <aside className={`sidebar ${mobileNavOpen?"mobile-open":""} ${sidebarCollapsed?"desktop-collapsed":""}`}>
      ...
      <nav aria-label="Main navigation">
        {navigationItems.map(([id,icon,label])=>
          <button key={id} className={`${view===id?"active":""} ${!isSharedMember&&(id==="calculator"||id==="settlement")?"mobile-secondary-nav":""}`.trim()} onClick={()=>chooseView(id)}><span>{icon}</span>{label}</button>)}
      </nav>
