...
function ExpenseModal({expense,players,games,onClose,onSave,onDelete}:{expense?:Expense;players:Player[];games:Game[];onClose:()=>void;onSave:(e:Omit<Expense,"id">)=>void;onDelete?:()=>void}) {
  ...
  return <div className="modal-backdrop"><form className="modal lineup-modal" onSubmit={e=>{e.preventDefault();if(canSave)onSave({label:label.trim(),amount:Number(amount),category,paidBy,gameId:split==="players"?gameId:undefined,split,date,participants:split==="appearances"?undefined:[...participants]})}}><ModalHead eyebrow={expense?"EDIT PAYMENT":"NEW PAYMENT"} title={expense?"Edit expense":"Add an expense"} description="Choose who paid and exactly how this cost should be shared." close={onClose}/><div className="form-grid"><label className="wide">Description<input autoFocus required value={label} onChange={e=>setLabel(e.target.value)} placeholder="e.g. Practice ground rental"/></label><label>Amount ($)<input required min=".01" step=".01" type="number" value={amount} onChange={e=>setAmount(e.target.value)}/></label><label>Expense date<input required type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label>Category<select value={category} onChange={e=>{const next=e.target.value;setCategory(next);if(!expense&&next==="League fee")setSplit("appearances")}}><option>Water</option><option>Fruits</option><option>IV</option><option>Practice</option><option>Team fund</option><option>League fee</option><option>Other</option></select></label><label>Paid by<select required value={paidBy} onChange={e=>setPaidBy(Number(e.target.value))}>{sortedPlayers.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><SplitFields split={split} setSplit={setSplit} gameId={gameId} setGameId={setGameId} games={games} allowAppearances/></div>{split==="custom"&&<ParticipantPicker players={sortedPlayers} selected={customPlayers} setSelected={setCustomPlayers} title="Select everyone who should share this expense"/>}{split==="appearances"&&<div className={totalAppearances?"split-note":"split-note warning"}><strong>{totalAppearances} completed-game appearances recorded</strong><span>{totalAppearances?"The amount will be weighted by each player’s completed games.":"Mark games Completed later; this fee will recalculate automatically."}</span></div>}<div className="modal-actions">{onDelete&&<button type="button" className="danger-action" onClick={onDelete}>Delete expense</button>}<span className={canSave?"ready":"warning"}>{split==="appearances"?`${totalAppearances} appearances`: `${participants.length} sharing`}</span><button type="button" className="ghost" onClick={onClose}>Cancel</button><button className="primary" disabled={!canSave}>{expense?"Save changes":"Add & split"}</button></div></form></div>;
}
...
function SplitFields({split,setSplit,gameId,setGameId,games,allowAppearances=false}:{split:ExpenseSplitMode;setSplit:(split:ExpenseSplitMode)=>void;gameId:number;setGameId:(id:number)=>void;games:Game[];allowAppearances?:boolean}) {
  return <><label>Who shares it?<select value={split} onChange={e=>setSplit(e.target.value as ExpenseSplitMode)}><option value="team">Full team roster</option>{allowAppearances&&<option value="appearances">By games played</option>}{games.length>0&&<option value="players">A game lineup</option>}<option value="custom">Custom players</option></select></label>{split==="players"&&<label>Game<select required value={gameId} onChange={e=>setGameId(Number(e.target.value))}>{games.map((g,i)=><option key={g.id} value={g.id}>Game {i+1} vs {g.opponent} · {g.players.length} players</option>)}</select></label>}</>;
}
