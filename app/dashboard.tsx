...
function CalculatorView({players,games,expenses,credits,balances}:{players:Player[];games:Game[];expenses:Expense[];credits:Credit[];balances:PlayerBalance[]}) {
  ...
  const rows=players.map(player=>{
    ...
    return {player,gamesPlayed,appearances,umpiredGames,umpiringDetails,umpiringCredit,weight:totalAppearances?appearances/totalAppearances:0,leagueFeeShare,refreshmentShare,otherExpenseShare,creditShare,total:leagueFeeShare+refreshmentShare+otherExpenseShare+creditShare};
  }).filter(row=>row.gamesPlayed||row.umpiringCredit>.004||row.total>.004);
  return <><div className="view-toolbar"><div><h2>League share calculator</h2><p>See exactly how games played, split eligibility, and umpiring waivers affect each player.</p></div></div><section className="calculator-formula"><div><span>Completed games</span><strong>{completed.length}</strong></div><div><span>Split-eligible appearances</span><strong>{totalAppearances}</strong></div><div><span>Appearance-weighted costs</span><strong>{money.format(weightedTotal)}</strong></div><p><b>Formula</b> Player share = expense × eligible appearances ÷ total eligible appearances. Players excluded from a game’s fair split still keep that game in their playing history. Umpiring waiver = games umpired × fixed credit per game.</p></section>{totalAppearances===0&&weightedTotal>0&&<div className="calculator-warning"><strong>No eligible completed appearances yet</strong><span>Mark games Completed and include at least one lineup player in their fair split.</span></div>}{rows.length?<div className="table-panel calculator-table"><table><thead><tr><th>Player</th><th>Games played</th><th>Split appearances</th><th>Games umpired</th><th>Umpiring waiver</th><th>Weight</th><th>League fee</th><th>Fruits / water</th><th>Other costs</th><th>Legacy credits funded</th><th>Total fair share</th></tr></thead><tbody>{rows.map(row=><tr key={row.player.id}><td><div className="player-cell"><span className="avatar" style={{background:row.player.color}}>{row.player.initials}</span><strong>{row.player.name}</strong></div></td><td>{row.gamesPlayed}</td><td>{row.appearances}</td><td><strong>{row.umpiredGames}</strong>{row.umpiringDetails.length>0&&<small className="umpired-games">{row.umpiringDetails.join(" · ")}</small>}</td><td className="positive">−{money.format(row.umpiringCredit)}</td><td>{(row.weight*100).toFixed(2)}%</td><td>{money.format(row.leagueFeeShare)}</td><td>{money.format(row.refreshmentShare)}</td><td>{money.format(row.otherExpenseShare)}</td><td>{money.format(row.creditShare)}</td><td><strong>{money.format(row.total)}</strong></td></tr>)}</tbody><tfoot><tr><td colSpan={10}>Calculated fair shares</td><td><strong>{money.format(balances.reduce((sum,balance)=>sum+balance.share,0))}</strong></td></tr></tfoot></table></div>:<MiniEmpty text="Add completed games and expenses to calculate player shares."/>}</>;
}
