...
export default function Dashboard({ user }: { user: { name: string; email: string; provider?:"google"|"phone"|"password"|"team" } }) {
  ...
  return <main className={`app-shell ${sidebarCollapsed?"sidebar-collapsed":""}`}>
    ...
    {modal==="payment" && league && league.status!=="Completed" && <SettlementPaymentModal players={players} balances={balances} suggestions={settlementTransfers(balances)} playerId={isSharedMember?memberPlayerId:undefined} onClose={()=>setModal(null)} onSave={payment=>{const pending=isSharedMember;updateLeague(l=>({...l,payments:[...(l.payments??[]),{...payment,id:Date.now(),recordedBy:user.email.toLowerCase(),status:pending?"pending":"confirmed"}]}));setModal(null);setView("settlement");history.replaceState({...history.state,wicketView:"settlement"},"");notify(pending?"Payment submitted for treasurer confirmation":"Payment confirmed and remaining balances recalculated")}}/>}
...
function SettlementView({team,league,balances,games,expenses,credits,payments,players,canManage,viewerPlayerId,onRecord,onCloseLeague,onConfirm,onDelete,exportCsv,notify}:{team:Team;league:League;balances:PlayerBalance[];games:Game[];expenses:Expense[];credits:Credit[];payments:SettlementPayment[];players:Player[];canManage:boolean;viewerPlayerId?:number;onRecord:()=>void;onCloseLeague:()=>void;onConfirm:(payment:SettlementPayment)=>void;onDelete:(payment:SettlementPayment)=>void;exportCsv:()=>void;notify:(s:string)=>void}){
