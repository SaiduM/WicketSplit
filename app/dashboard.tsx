...
type SettlementPayment = { id: number; date: string; fromPlayerId: number; toPlayerId: number; amount: number; note?: string; recordedBy?: string; status?: "pending"|"confirmed"; confirmedBy?: string; confirmedAt?: string };
...
type Account = { registered: boolean; name: string; teams: Team[]; earlyAccess?:{status:"none"|"pending"|"approved"|"rejected";isAdmin?:boolean} };
type View = "overview" | "roster" | "users" | "games" | "expenses" | "settlement" | "leagues";
...
export default function Dashboard({ user }: { user: { name: string; email: string; provider?:"google"|"phone"|"password"|"team" } }) {
  ...
  useEffect(()=>{
    ...
    const restoreView=(event:PopStateEvent)=>{
      ...
      setView(next&&["overview","roster","leagues","games","expenses","settlement"].includes(next)?next:"overview");
  ...
  const isTeamMember = team?.access?.role==="member";
  ...
  const balances = useMemo(() => players.map(player => {
    ...
    const confirmedPayments=payments.filter(payment=>payment.status!=="pending");
    const sent = confirmedPayments.filter(payment=>payment.fromPlayerId===player.id).reduce((sum,payment)=>sum+payment.amount,0);
    const received = confirmedPayments.filter(payment=>payment.toPlayerId===player.id).reduce((sum,payment)=>sum+payment.amount,0);
  ...
  if (!account.registered) {
    ...
    if(account.earlyAccess?.status!=="approved")return <EarlyAccessGate user={user} initialStatus={account.earlyAccess?.status??"none"}/>;
  ...
  function exportCsv() {
    ...
    const rows = [["Team",team.name],["League",league.name],["Season",league.season],[],
      ...
      ...payments.filter(payment=>payment.status!=="pending").map(payment=>[payment.date,players.find(player=>player.id===payment.fromPlayerId)?.name??"Unknown",players.find(player=>player.id===payment.toPlayerId)?.name??"Unknown",payment.amount.toFixed(2),payment.note??"",payment.confirmedBy??payment.recordedBy??""]),
  ...
  const navigationItems:ReadonlyArray<readonly [View,string,string]>=isTeamMember
  ...
  return <main className={`app-shell ${sidebarCollapsed?"sidebar-collapsed":""}`}>
    ...
    <aside className={`sidebar ${mobileNavOpen?"mobile-open":""} ${sidebarCollapsed?"desktop-collapsed":""}`}>
      ...
      <div className="side-bottom">
        <div className="profile-wrap" ref={profileMenuRef}>
          {profileMenu&&<div className="profile-menu"><div className="profile-menu-title"><strong>Account</strong><span>{team?.name}</span></div><dl><div><dt>Name</dt><dd>{accountDisplayName}</dd></div><div><dt>Role</dt><dd><span className="account-role">{accountRole}</span></dd></div><div><dt>Email</dt><dd>{accountEmail}</dd></div><div><dt>Phone</dt><dd>{accountPhone}</dd></div></dl>{isTreasurer&&!isSharedMember&&<a href="/backup"><span>⇩</span>Backup & recovery</a>}<a href="/feedback"><span>?</span>Report a problem</a>{account.earlyAccess?.isAdmin&&<><a className="admin-access-link" href="/early-access"><span>◇</span>Early-access requests</a><a className="admin-access-link" href="/feedback-admin"><span>!</span>Feedback reports</a></>}<a href="/api/auth/logout" aria-label="Log out"><span>↪</span>Log out</a></div>}
    ...
    <section className="workspace" onTouchStart={beginEdgeSwipe} onTouchEnd={finishEdgeSwipe}>
      ...
      {!team ? <EmptyState icon="♙" title={isSharedMember?"Team unavailable":"Register your first team"} text={isSharedMember?"Ask your treasurer for the current team link and PIN.":"Create a team workspace, add its roster, then organize expenses by league."} action={isSharedMember?undefined:"Register team"} onAction={isSharedMember?undefined:()=>setModal("team")}/> :
       !league && view!=="roster" && view!=="users" && view!=="leagues" ? <EmptyState icon="▤" title={isSharedMember?"No league available":"Create your first league"} text={isSharedMember?"Your treasurer has not created a league yet.":"A team can have as many leagues and seasons as you need."} action={isSharedMember?undefined:"Create league"} onAction={isSharedMember?undefined:()=>setModal("league")}/> : <>
        ...
        {view==="leagues" && <LeaguesView team={team} canManage={isTreasurer} activeId={league?.id} onSelect={id=>{setLeagueId(id);chooseView("overview")}} onAdd={()=>setModal("league")} onSync={()=>setModal("cricclubs")} onEdit={candidate=>candidate.status==="Completed"?notify("Completed leagues are locked for reference"):setEditingLeague(candidate)}/>}
        {view==="games" && league && <GamesView games={games} expenses={expenses} credits={credits} players={players} canManage={isTreasurer&&league.status!=="Completed"} onAdd={()=>setModal("game")} onScreenshots={()=>setModal("screenshots")} onSync={()=>setModal("cricclubs")} onRoster={()=>chooseView("roster")} onChange={next=>updateLeague(l=>({...l,games:next}))} notify={notify}/>}
        {view==="expenses" && league && <ExpensesView expenses={expenses} credits={credits} players={players} games={games} canManage={isTreasurer&&league.status!=="Completed"} memberEmail={user.email} onAdd={()=>setModal("expense")} onCredit={()=>setModal("credit")} onRoster={()=>chooseView("roster")} onEdit={setEditingExpense} onDelete={expense=>{if(confirm(`Delete “${expense.label}”? This will recalculate every balance.`)){updateLeague(current=>({...current,expenses:current.expenses.filter(entry=>entry.id!==expense.id)}));notify("Expense deleted and balances recalculated")}}} onEditCredit={setEditingCredit}/>}
        {view==="settlement" && league && <SettlementView
          team={team} league={league} balances={balances} games={games} expenses={expenses} credits={credits} payments={payments} players={players}
          canManage={isTreasurer&&league.status!=="Completed"} viewerPlayerId={isTeamMember?memberPlayerId:undefined} canSubmitPayment={isTeamMember&&!isSharedMember&&!user.email.startsWith("phone:")} onRecord={()=>setModal("payment")} onCloseLeague={()=>{if(confirm(`Close ${league.name}? This permanently locks its games, expenses, credits, and settlements as read-only reference.`)){updateLeague(current=>({...current,status:"Completed"}));setView("settlement");notify("League closed and locked for reference")}}} onReopenLeague={()=>{if(confirm(`Reopen ${league.name} so its outstanding settlements can be completed?`)){updateLeague(current=>({...current,status:"Active"}));notify("League reopened — finish the pending settlements before locking it again")}}}
          onConfirm={payment=>{const sender=balances.find(balance=>balance.id===payment.fromPlayerId);const receiver=balances.find(balance=>balance.id===payment.toPlayerId);const maximum=Math.max(0,Math.min(-(sender?.balance??0),receiver?.balance??0));if(payment.amount>maximum+.005){notify("This payment no longer matches the current balances. Reject it and ask the player to submit the updated amount.");return}updateLeague(current=>({...current,payments:(current.payments??[]).map(entry=>entry.id===payment.id?{...entry,status:"confirmed",confirmedBy:user.email.toLowerCase(),confirmedAt:new Date().toISOString()}:entry)}));notify("Payment confirmed and balances recalculated")}}
          onDelete={payment=>{const prompt=payment.status==="pending"?`Reject this ${money.format(payment.amount)} payment submission? It has not changed any balances.`:`Delete this ${money.format(payment.amount)} payment record? Remaining balances will be recalculated.`;if(confirm(prompt)){updateLeague(current=>({...current,payments:(current.payments??[]).filter(entry=>entry.id!==payment.id)}));notify(payment.status==="pending"?"Payment submission rejected":"Payment record deleted and balances restored")}}}
          exportCsv={exportCsv} notify={notify}/>}
      </>}
    </section>
    ...
    {editingLeague && team && editingLeague.status!=="Completed" && <LeagueModal league={editingLeague} onClose={()=>setEditingLeague(null)} onDelete={()=>deleteLeague(editingLeague)} onSave={(name,season,status)=>{if(status==="Completed"){notify("Close a fully settled league from Settlement");return}updateTeam(t=>({...t,leagues:t.leagues.map(l=>l.id===editingLeague.id?{...l,name,season,status}:l)}));setEditingLeague(null);notify("League details updated")}}/>}
    ...
    {modal==="payment" && league && league.status!=="Completed" && (isTreasurer||(isTeamMember&&!isSharedMember&&!user.email.startsWith("phone:"))) && <SettlementPaymentModal players={players} balances={balances} suggestions={settlementTransfers(balances)} playerId={isTeamMember?memberPlayerId:undefined} onClose={()=>setModal(null)} onSave={payment=>{const pending=isTeamMember;updateLeague(l=>({...l,payments:[...(l.payments??[]),{...payment,id:Date.now(),recordedBy:user.email.toLowerCase(),status:pending?"pending":"confirmed"}]}));setModal(null);setView("settlement");history.replaceState({...history.state,wicketView:"settlement"},"");notify(pending?"Payment submitted for treasurer confirmation":"Payment confirmed and remaining balances recalculated")}}/>}
...
function CalculatorView({players,games,expenses,credits}:{players:Player[];games:Game[];expenses:Expense[];credits:Credit[];balances:PlayerBalance[]}) {
  const completed=fundedGames(games);
  const teamCost=expenses.filter(usesAppearanceSplit).reduce((sum,expense)=>sum+expense.amount,0)+credits.filter(isUmpiringWaiver).reduce((sum,credit)=>sum+credit.amount,0);
  const gameCost=completed.length?teamCost/completed.length:0;
  const rows=completed.map((game,index)=>{const eligibleIds=splitEligiblePlayers(game);const eligible=eligibleIds.map(id=>players.find(player=>player.id===id)?.name).filter((name):name is string=>Boolean(name)).sort((a,b)=>a.localeCompare(b));return {game,index,eligible,share:eligible.length?gameCost/eligible.length:0}});
  return rows.length?<section className="per-game-breakdown compact-game-shares"><div className="panel-head"><div><h2>Player share for each game</h2><p>The team-funded cost is split equally across completed games, then equally among the listed players.</p></div></div><div className="table-panel"><table><thead><tr><th>Game</th><th>Players sharing this game</th><th>Game cost</th><th>Each player</th></tr></thead><tbody>{rows.map(row=><tr key={row.game.id}><td><strong>Game {row.index+1}: {row.game.opponent}</strong></td><td><span className="game-share-names">{row.eligible.join(", ")}</span></td><td>{money.format(gameCost)}</td><td><strong>{money.format(row.share)}</strong></td></tr>)}</tbody></table></div></section>:<MiniEmpty text="Add completed games and team-funded expenses to see per-game player shares."/>;
}

function SettlementView({team,league,balances,games,expenses,credits,payments,players,canManage,viewerPlayerId,canSubmitPayment,onRecord,onCloseLeague,onReopenLeague,onConfirm,onDelete,exportCsv,notify}:{team:Team;league:League;balances:PlayerBalance[];games:Game[];expenses:Expense[];credits:Credit[];payments:SettlementPayment[];players:Player[];canManage:boolean;viewerPlayerId?:number;canSubmitPayment:boolean;onRecord:()=>void;onCloseLeague:()=>void;onReopenLeague:()=>void;onConfirm:(payment:SettlementPayment)=>void;onDelete:(payment:SettlementPayment)=>void;exportCsv:()=>void;notify:(s:string)=>void}){
  const [detail,setDetail]=useState<number|null>(null);const [balanceFilter,setBalanceFilter]=useState<"all"|"pending">("all");const [section,setSection]=useState<"summary"|"games"|"payments">("summary");
  ...
  const playerView=!canManage&&viewerPlayerId!==undefined;const transfers=settlementTransfers(balances);const visibleTransfers=playerView?transfers.filter(transfer=>transfer.fromPlayerId===viewerPlayerId||transfer.toPlayerId===viewerPlayerId):transfers;const shown=playerView?balances.filter(balance=>balance.id===viewerPlayerId):balances.filter(balance=>balanceFilter==="all"||Math.abs(balance.balance)>.005);const confirmedPayments=payments.filter(payment=>payment.status!=="pending");const pendingPayments=payments.filter(payment=>payment.status==="pending");const visiblePayments=(playerView?confirmedPayments.filter(payment=>payment.fromPlayerId===viewerPlayerId||payment.toPlayerId===viewerPlayerId):confirmedPayments);const visiblePending=playerView?pendingPayments.filter(payment=>payment.fromPlayerId===viewerPlayerId):pendingPayments;const canPlayerSettle=playerView&&canSubmitPayment&&transfers.some(transfer=>transfer.fromPlayerId===viewerPlayerId)&&!pendingPayments.some(payment=>payment.fromPlayerId===viewerPlayerId);
  ...
  const paidTotal=visiblePayments.reduce((sum,payment)=>sum+payment.amount,0);const remainingTotal=playerView?Math.abs(shown[0]?.balance??0):balances.filter(balance=>balance.balance<-.005).reduce((sum,balance)=>sum+Math.abs(balance.balance),0);const fullySettled=transfers.length===0&&pendingPayments.length===0&&balances.every(balance=>Math.abs(balance.balance)<=.005);const completionNeedsAttention=league.status==="Completed"&&!fullySettled;
  return <><div className="view-toolbar"><div><h2>{playerView?"My settlement breakdown":"League settlement"}</h2><p>{league.status==="Completed"?"This league is closed and preserved as read-only reference.":playerView?"See exactly how your costs, credits, and payments produce your balance.":"Balances, game shares, and payments are separated into simple views."}</p></div>{playerView?<>{canPlayerSettle&&<button className="primary" onClick={onRecord}>✓ I paid — settle up</button>}{!canSubmitPayment&&league.status!=="Completed"&&<a className="ghost" href="/api/auth/logout">Sign in with email to settle</a>}</>:<div className="toolbar-actions">{canManage&&transfers.length>0&&<button className="primary" onClick={onRecord}>✓ Record payment</button>}{canManage&&fullySettled&&<button className="primary close-league" onClick={onCloseLeague}>Lock completed league</button>}<button className="ghost" onClick={shareSettlement}>↗ Share remaining</button><button className="ghost" onClick={exportCsv}>↓ Download CSV</button></div>}</div>
    {league.status==="Completed"&&<div className={`league-locked-banner ${completionNeedsAttention?"needs-attention":""}`}><strong>{completionNeedsAttention?"! Completion needs attention":"✓ League completed and locked"}</strong><span>{completionNeedsAttention?"This league still has outstanding balances or a payment awaiting confirmation. Reopen it to finish settlement.":"Games, expenses, credits, and settlement records can be viewed or exported but no longer edited."}</span>{completionNeedsAttention&&!playerView&&<button className="ghost" onClick={onReopenLeague}>Reopen league</button>}</div>}
    <nav className="settlement-tabs" aria-label="Settlement sections"><button className={section==="summary"?"active":""} onClick={()=>setSection("summary")}>Balances</button><button className={section==="games"?"active":""} onClick={()=>setSection("games")}>Shares by game</button><button className={section==="payments"?"active":""} onClick={()=>setSection("payments")}>Payments{pendingPayments.length?` (${pendingPayments.length})`:""}</button></nav>
    {section==="summary"&&<>
    ...
    {section==="games"&&<CalculatorView players={players} games={games} expenses={expenses} credits={credits} balances={balances}/>}
    {section==="payments"&&<>
    {visiblePending.length>0&&<section className="payment-history pending-payments"><div className="panel-head"><div><h2>{playerView?"Payment awaiting confirmation":"Payments to confirm"}</h2><p>{playerView?"Your balance changes after a treasurer confirms receipt.":`${visiblePending.length} player ${visiblePending.length===1?"submission":"submissions"} awaiting review`}</p></div></div><div className="table-panel"><table><thead><tr><th>Date</th><th>From</th><th>To</th><th>Amount</th><th>Reference</th><th></th></tr></thead><tbody>{visiblePending.map(payment=><tr key={payment.id}><td>{new Date(payment.date+"T12:00").toLocaleDateString()}</td><td>{players.find(player=>player.id===payment.fromPlayerId)?.name??"Unknown"}</td><td>{players.find(player=>player.id===payment.toPlayerId)?.name??"Unknown"}</td><td><strong>{money.format(payment.amount)}</strong></td><td>{payment.note||"—"}</td><td>{canManage&&<div className="row-actions"><button className="table-edit" onClick={()=>onConfirm(payment)}>Confirm received</button><button className="table-delete" onClick={()=>onDelete(payment)}>Reject</button></div>}</td></tr>)}</tbody></table></div></section>}
...
function LeagueModal({league,onClose,onSave,onDelete}:{league?:League;onClose:()=>void;onSave:(name:string,season:string,status:"Active"|"Completed")=>void;onDelete?:()=>void}) {
  const [name,setName]=useState(league?.name??""); const [season,setSeason]=useState(league?.season??new Date().getFullYear().toString()); return <div className="modal-backdrop"><form className="modal small-modal" onSubmit={e=>{e.preventDefault();onSave(name,season,league?.status??"Active")}}><ModalHead eyebrow={league?"EDIT LEAGUE":"NEW LEAGUE"} title={league?"Edit league details":"Create a league"} description={league?"Close and permanently lock the league from Settlement after every balance reaches zero.":"Games, expenses, and settlement stay separate per league."} close={onClose}/><div className="form-grid"><label className="wide">League name<input autoFocus required value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Summer League"/></label><label className="wide">Season / year<input required value={season} onChange={e=>setSeason(e.target.value)} placeholder="2026"/></label></div><div className="modal-actions">{onDelete&&<button type="button" className="danger-action" onClick={onDelete}>Delete league</button>}<button type="button" className="ghost" onClick={onClose}>Cancel</button><button className="primary">{league?"Save changes":"Create league"}</button></div></form></div>;
}
...
function SettlementPaymentModal({players,balances,suggestions,playerId,onClose,onSave}:{players:Player[];balances:PlayerBalance[];suggestions:ReturnType<typeof settlementTransfers>;playerId?:number;onClose:()=>void;onSave:(payment:Omit<SettlementPayment,"id"|"recordedBy">)=>void}) {
  ...
  const allowedSuggestions=playerId===undefined?suggestions:suggestions.filter(suggestion=>suggestion.fromPlayerId===playerId);
  const first=allowedSuggestions[0];
  const [fromPlayerId,setFromPlayerId]=useState(playerId??first?.fromPlayerId??debtors[0]?.id??0);
  ...
  const chooseSuggestion=(index:number)=>{const suggestion=allowedSuggestions[index];if(!suggestion)return;setFromPlayerId(suggestion.fromPlayerId);setToPlayerId(suggestion.toPlayerId);setAmount(suggestion.amount.toFixed(2))};
  const playerSubmission=playerId!==undefined;
  return <div className="modal-backdrop"><form className="modal lineup-modal" onSubmit={event=>{event.preventDefault();if(valid)onSave({date,fromPlayerId,toPlayerId,amount:numericAmount,note:note.trim()})}}><ModalHead eyebrow={playerSubmission?"SETTLE UP":"CONFIRMED REPAYMENT"} title={playerSubmission?"Tell your treasurer you paid":"Record a received payment"} description={playerSubmission?"Submit the payment you sent. Your balance changes only after a treasurer confirms it was received.":"Use this only after the receiver confirms the money arrived. It reduces both players’ remaining settlement balances."} close={onClose}/>{allowedSuggestions.length>0&&<div className="payment-suggestions"><strong>{playerSubmission?"Your current payment":"Current suggestions"}</strong><div>{allowedSuggestions.map((suggestion,index)=><button type="button" key={`${suggestion.fromPlayerId}-${suggestion.toPlayerId}`} onClick={()=>chooseSuggestion(index)}><span>{suggestion.from} → {suggestion.to}</span><b>{money.format(suggestion.amount)}</b></button>)}</div></div>}<div className="form-grid"><label>Money sent by<select required disabled={playerSubmission} value={fromPlayerId} onChange={event=>{const id=Number(event.target.value);setFromPlayerId(id);const next=balances.find(balance=>balance.id===id);setAmount(Math.min(-(next?.balance??0),toBalance?.balance??0).toFixed(2))}}>{debtors.filter(player=>!playerSubmission||player.id===playerId).map(player=><option key={player.id} value={player.id}>{player.name} · owes {money.format(-player.balance)}</option>)}</select></label><label>Money received by<select required value={toPlayerId} onChange={event=>{const id=Number(event.target.value);setToPlayerId(id);const next=balances.find(balance=>balance.id===id);setAmount(Math.min(-(fromBalance?.balance??0),next?.balance??0).toFixed(2))}}>{creditors.map(player=><option key={player.id} value={player.id}>{player.name} · due {money.format(player.balance)}</option>)}</select></label><label>{playerSubmission?"Amount sent ($)":"Amount received ($)"}<input required min=".01" max={maximum.toFixed(2)} step=".01" type="number" value={amount} onChange={event=>setAmount(event.target.value)}/><small className="field-help">Maximum for this pair: {money.format(maximum)}</small></label><label>{playerSubmission?"Payment date":"Received date"}<input required type="date" value={date} onChange={event=>setDate(event.target.value)}/></label><label className="wide">Reference / note (optional)<input maxLength={240} value={note} onChange={event=>setNote(event.target.value)} placeholder="e.g. Zelle reference, cash, bank transfer"/></label></div><div className="payment-confirmation"><strong>{playerSubmission?"This submits a confirmation request—it does not move money or change balances yet.":"This records confirmation—it does not move money."}</strong><span>{valid?(playerSubmission?`Your treasurer will review this ${money.format(numericAmount)} payment.`:`${players.find(player=>player.id===fromPlayerId)?.name} will owe ${money.format(Math.max(0,-(fromBalance?.balance??0)-numericAmount))} after this payment.`):"Choose a valid payer, receiver, and amount within their remaining balances."}</span></div><div className="modal-actions"><span className={valid?"ready":"warning"}>{valid?playerSubmission?"Ready to submit":"Ready to confirm":"Check payment"}</span><button type="button" className="ghost" onClick={onClose}>Cancel</button><button className="primary" disabled={!valid}>{playerSubmission?"Submit payment":"Confirm payment received"}</button></div></form></div>;
}
