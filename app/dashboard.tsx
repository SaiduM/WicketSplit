      ...
      {!team ? <EmptyState icon="♙" title={isSharedMember?"Team unavailable":"Register your first team"} text={isSharedMember?"Ask your treasurer for the current team link and PIN.":"Create a team workspace, add its roster, then organize expenses by league."} action={isSharedMember?undefined:"Register team"} onAction={isSharedMember?undefined:()=>setModal("team")}/> :
       !league && view!=="roster" && view!=="users" && view!=="leagues" ? <EmptyState icon="▤" title={isSharedMember?"No league available":"Create your first league"} text={isSharedMember?"Your treasurer has not created a league yet.":"A team can have as many leagues and seasons as you need."} action={isSharedMember?undefined:"Create league"} onAction={isSharedMember?undefined:()=>setModal("league")}/> : <>
        ...
        {view==="settlement" && league && <SettlementView
          ...
          canManage={isTreasurer&&league.status!=="Completed"} viewerPlayerId={isTeamMember?memberPlayerId:undefined} canSubmitPayment={isTeamMember&&!isSharedMember&&!user.email.startsWith("phone:")} onRecord={()=>setModal("payment")} onCloseLeague={()=>{if(confirm(`Close ${league.name}? This permanently locks its games, expenses, credits, and settlements as read-only reference.`)){updateLeague(current=>({...current,status:"Completed"}));setView("settlement");notify("League closed and locked for reference")}}}
...
function SettlementView({team,league,balances,games,expenses,credits,payments,players,canManage,viewerPlayerId,canSubmitPayment,onRecord,onCloseLeague,onConfirm,onDelete,exportCsv,notify}:{team:Team;league:League;balances:PlayerBalance[];games:Game[];expenses:Expense[];credits:Credit[];payments:SettlementPayment[];players:Player[];canManage:boolean;viewerPlayerId?:number;canSubmitPayment:boolean;onRecord:()=>void;onCloseLeague:()=>void;onConfirm:(payment:SettlementPayment)=>void;onDelete:(payment:SettlementPayment)=>void;exportCsv:()=>void;notify:(s:string)=>void}){
  ...
  return <><div className="view-toolbar"><div><h2>{playerView?"My settlement breakdown":"League settlement"}</h2><p>{league.status==="Completed"?"This league is closed and preserved as read-only reference.":playerView?"See exactly how your costs, credits, and payments produce your balance.":"Balances, game shares, and payments are separated into simple views."}</p></div>{playerView?<>{canPlayerSettle&&<button className="primary" onClick={onRecord}>✓ I paid — settle up</button>}{!canSubmitPayment&&league.status!=="Completed"&&<a className="ghost" href="/api/auth/logout">Sign in with email to settle</a>}</>:<div className="toolbar-actions">{canManage&&transfers.length>0&&<button className="primary" onClick={onRecord}>✓ Record payment</button>}{canManage&&fullySettled&&<button className="primary close-league" onClick={onCloseLeague}>Lock completed league</button>}<button className="ghost" onClick={shareSettlement}>↗ Share remaining</button><button className="ghost" onClick={exportCsv}>↓ Download CSV</button></div>}</div>
    {league.status==="Completed"&&<div className="league-locked-banner"><strong>✓ League completed and locked</strong><span>Games, expenses, credits, and settlement records can be viewed or exported but no longer edited.</span></div>}
