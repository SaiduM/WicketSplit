...
type Expense = { id: number; date: string; label: string; category: string; amount: number; paidBy: number; gameId?: number; split: ExpenseSplitMode; participants?: number[] };
...
export default function Dashboard({ user }: { user: { name: string; email: string } }) {
  ...
  return <main className="app-shell">
    ...
    {modal==="expense" && league && <ExpenseModal existingExpenses={expenses} leagueFeeExists={expenses.some(e=>e.category==="League fee")} players={players} games={games} onClose={()=>setModal(null)} onSave={expense=>{updateLeague(l=>({...l,expenses:[...l.expenses,{...expense,id:Date.now()}]}));setModal(null);notify("Expense added and split recalculated")}}/>}
...
function ExpenseModal({expense,existingExpenses,leagueFeeExists,players,games,onClose,onSave,onDelete}:{expense?:Expense;existingExpenses:Expense[];leagueFeeExists:boolean;players:Player[];games:Game[];onClose:()=>void;onSave:(e:Omit<Expense,"id">)=>void;onDelete?:()=>void}) {
