...
export default function Dashboard({ user }: { user: { name: string; email: string } }) {
  ...
  const deletePlayer = async (player: Player) => {
    if(!team||!confirm(`Delete ${player.name} from ${team.name}? This is only allowed when they have no games, expenses, credits, or team access.`))return;
  ...
  function exportCsv() {
    ...
    const exportBalances = balances.filter(b => Math.abs(b.paid) > .005 || Math.abs(b.credit) > .005 || Math.abs(b.share) > .005);
    const rows = [["Team",team.name],["League",league.name],["Season",league.season],[],
      ...
      ...exportBalances.map(b => [b.name,games.filter(g=>g.status==="Completed"&&g.players.includes(b.id)).length,b.paid.toFixed(2),b.credit.toFixed(2),b.share.toFixed(2),b.balance.toFixed(2)]),
    ...
    <section className="workspace">
      ...
      {!team ? <EmptyState icon="♙" title="Register your first team" text="Create a team workspace, add its roster, then organize expenses by league." action="Register team" onAction={()=>setModal("team")}/> :
       !league && view!=="roster" && view!=="leagues" ? <EmptyState icon="▤" title="Create your first league" text="A team can have as many leagues and seasons as you need." action="Create league" onAction={()=>setModal("league")}/> : <>
        ...
        {view==="roster" && <RosterView team={team} canManage={isTreasurer} onInvite={setInviteTarget} onAdd={()=>setModal("player")} onEdit={setEditingPlayer} onDelete={deletePlayer}/>}
