...
function CalculatorView({players,games,expenses,credits,balances}:{players:Player[];games:Game[];expenses:Expense[];credits:Credit[];balances:PlayerBalance[]}) {
  ...
  const weightedTotal=weightedExpenses.reduce((sum,expense)=>sum+expense.amount,0)+umpiringTotal;
  const rows=players.map(player=>{
    ...
    const umpiringCostShare=umpiringCredits.length||umpiringTotal?credits.filter(isUmpiringWaiver).reduce((sum,credit)=>sum+appearanceShare(credit.amount,player.id,games),0):0;
