...
const appearanceShare = (amount:number,playerId:number,games:Game[]) => {
  ...
  const appearances=completed.filter(game=>splitEligiblePlayers(game).includes(playerId)).length;
...
const splitDescription = (entry:{split:ExpenseSplitMode;category?:string;participants?:number[];gameId?:number;kind?:string},players:Player[],games:Game[]) => {
  if(entry.kind==="umpiring-waiver") return `Team funded · ${splitAppearanceCount(games)} eligible appearances`;
  if(entry.split==="appearances"||(entry.category&&appearanceCategories.has(entry.category))) return `By games played (${splitAppearanceCount(games)} eligible appearances)`;
...
const creditSplitDescription = (credit:Credit,players:Player[],games:Game[]) =>
  isUmpiringWaiver(credit) ? `Team funded · ${splitAppearanceCount(games)} eligible appearances` : splitDescription(credit,players,games);
...
export default function Dashboard({ user }: { user: { name: string; email: string; provider?:"google"|"phone"|"password"|"team" } }) {
  ...
  const balances = useMemo(() => players.map(player => {
    ...
    const creditShare = credits.reduce((sum,e) => sum + (isUmpiringWaiver(e)?appearanceShare(e.amount,player.id,games):playerShare(e.amount,e.participants??[],player.id)),0);
  ...
  function exportCsv() {
    ...
    const eligibleAppearances=splitAppearanceCount(completedGames);
    ...
    const perAppearance=eligibleAppearances?appearancePool/eligibleAppearances:0;
    const rows = [["Team",team.name],["League",league.name],["Season",league.season],[],
      ...
      ...completedGames.map((game,index)=>{const eligible=splitEligiblePlayers(game);const excluded=(game.excludedFromSplit??[]).map(id=>players.find(player=>player.id===id)?.name).filter(Boolean).join(", ");return [`Game ${index+1}: ${game.opponent}`,game.players.length,eligible.length,excluded,(eligible.length*perAppearance).toFixed(2),perAppearance.toFixed(2)]}),
...
function PersonalHome({player,players,games,credits,balances,onLink,setView,onPlayer,onAddExpense}:{player?:Player;players:Player[];games:Game[];credits:Credit[];balances:PlayerBalance[];onLink:(playerId:number)=>Promise<void>;setView:(view:View)=>void;onPlayer:()=>void;onAddExpense?:()=>void}) {
...
function CalculatorView({players,games,expenses,credits,balances}:{players:Player[];games:Game[];expenses:Expense[];credits:Credit[];balances:PlayerBalance[]}) {
  ...
  const costPerAppearance=totalAppearances?weightedTotal/totalAppearances:0;
  const perGame=completed.map((game,index)=>{
    ...
    const ratio=totalAppearances?eligibleIds.length/totalAppearances:0;
    return {game,index,eligibleCount:eligibleIds.length,excludedNames,cost:weightedTotal*ratio,leagueFee:leagueFeeTotal*ratio,refreshments:refreshmentTotal*ratio,umpiring:umpiringTotal*ratio};
  });
  const rows=players.map(player=>{
    ...
    const umpiringCostShare=credits.filter(isUmpiringWaiver).reduce((sum,credit)=>sum+appearanceShare(credit.amount,player.id,games),0);
    const creditShare=credits.filter(credit=>!isUmpiringWaiver(credit)).reduce((sum,credit)=>sum+playerShare(credit.amount,credit.participants??[],player.id),0);
    return {player,gamesPlayed,appearances,umpiredGames,umpiringDetails,umpiringCredit,umpiringCostShare,weight:totalAppearances?appearances/totalAppearances:0,leagueFeeShare,refreshmentShare,otherExpenseShare,creditShare,total:leagueFeeShare+refreshmentShare+otherExpenseShare+umpiringCostShare+creditShare};
  }).filter(row=>row.gamesPlayed||row.umpiringCredit>.004||row.total>.004);
  return <>
    ...
    <section className="calculator-formula"><div><span>Completed games</span><strong>{completed.length}</strong></div><div><span>Split-eligible appearances</span><strong>{totalAppearances}</strong></div><div><span>Appearance-weighted team costs</span><strong>{money.format(weightedTotal)}</strong></div><p><b>How umpiring credits work</b> The full umpiring credit is added to the team cost pool. Each player’s share = team cost pool × their eligible appearances ÷ total eligible appearances. Each eligible appearance currently costs {money.format(costPerAppearance)}.</p></section>
    {totalAppearances===0&&weightedTotal>0&&<div className="calculator-warning"><strong>No eligible completed appearances yet</strong><span>Mark games Completed and include at least one lineup player before adding appearance-funded credits.</span></div>}
    {perGame.length>0&&<section className="per-game-breakdown"><div className="panel-head"><div><h2>Per-game breakdown</h2><p>Informational view of the current appearance-weighted calculation. It does not change anyone’s share.</p></div></div><div className="table-panel"><table><thead><tr><th>Game</th><th>Players</th><th>Payers</th><th>Excluded</th><th>Allocated cost</th><th>$/payer</th></tr></thead><tbody>{perGame.map(row=><tr key={row.game.id}><td><strong>Game {row.index+1}: {row.game.opponent}</strong><small>{[row.leagueFee>.004&&`League ${money.format(row.leagueFee)}`,row.refreshments>.004&&`Fruits / Water ${money.format(row.refreshments)}`,row.umpiring>.004&&`Umpiring ${money.format(row.umpiring)}`].filter(Boolean).join(" · ")||"No appearance-weighted costs"}</small></td><td>{row.game.players.length}</td><td><strong>{row.eligibleCount}</strong></td><td>{row.excludedNames.length?<span className="excluded-list">{row.excludedNames.join(", ")}</span>:"—"}</td><td><strong>{money.format(row.cost)}</strong></td><td>{money.format(costPerAppearance)}</td></tr>)}</tbody><tfoot><tr><td colSpan={4}>Appearance-weighted pool represented</td><td><strong>{money.format(perGame.reduce((sum,row)=>sum+row.cost,0))}</strong></td><td>{totalAppearances} appearances</td></tr></tfoot></table></div></section>}
    {rows.length?<div className="table-panel calculator-table"><table><thead><tr><th>Player</th><th>Games played</th><th>Split appearances</th><th>Games umpired</th><th>Umpiring credit</th><th>Weight</th><th>League fee</th><th>Fruits / water</th><th>Umpiring cost share</th><th>Other costs</th><th>Legacy credits funded</th><th>Total fair share</th></tr></thead><tbody>{rows.map(row=><tr key={row.player.id}><td><div className="player-cell"><span className="avatar" style={{background:row.player.color}}>{row.player.initials}</span><strong>{row.player.name}</strong></div></td><td>{row.gamesPlayed}</td><td>{row.appearances}</td><td><strong>{row.umpiredGames}</strong>{row.umpiringDetails.length>0&&<small className="umpired-games">{row.umpiringDetails.join(" · ")}</small>}</td><td className="positive">+{money.format(row.umpiringCredit)}</td><td>{(row.weight*100).toFixed(2)}%</td><td>{money.format(row.leagueFeeShare)}</td><td>{money.format(row.refreshmentShare)}</td><td>{money.format(row.umpiringCostShare)}</td><td>{money.format(row.otherExpenseShare)}</td><td>{money.format(row.creditShare)}</td><td><strong>{money.format(row.total)}</strong></td></tr>)}</tbody><tfoot><tr><td colSpan={11}>Calculated fair shares</td><td><strong>{money.format(balances.reduce((sum,balance)=>sum+balance.share,0))}</strong></td></tr></tfoot></table></div>:<MiniEmpty text="Add completed games and expenses to calculate player shares."/>}
  </>;
}
...
function PlayerBreakdown({player,expenses,credits,games,players,onClose}:{player:PlayerBalance;expenses:Expense[];credits:Credit[];games:Game[];players:Player[];onClose:()=>void}) {
  ...
  const creditRows=credits.flatMap(credit=>isUmpiringWaiver(credit)
    ? [{label:`${creditDisplayLabel(credit)} · team-funded share`,share:appearanceShare(credit.amount,player.id,games)},...(credit.playerId===player.id?[{label:`${creditDisplayLabel(credit)} · credit received`,share:-credit.amount}]:[])]
...
function TeamMemberAccessModal({team,onClose,notify}:{team:Team;onClose:()=>void;notify:(message:string)=>void}){
  ...
  const message=access?`🏏 Join ${team.name} on WicketSplit\n\nOpen this private link:\n${access.url}\n\nTeam PIN: ${access.pin}\n\nChoose your own name from the roster. You can view team records and add expenses you paid. Please do not forward this link or PIN.`:"";
...
function ExpenseModal({expense,memberPlayerId,existingExpenses,leagueFeeExists,players,games,onAddPlayer,onClose,onSave,onDelete}:{expense?:Expense;memberPlayerId?:number;existingExpenses:Expense[];leagueFeeExists:boolean;players:Player[];games:Game[];onAddPlayer?:(name:string)=>number;onClose:()=>void;onSave:(e:Omit<Expense,"id">)=>void;onDelete?:()=>void}) {
  ...
  const totalAppearances=splitAppearanceCount(games); const canSave=Boolean(category&&paidBy&&(split==="appearances"||participants.length>0));
  ...
  return <div className="modal-backdrop"><form className="modal lineup-modal" onSubmit={e=>{e.preventDefault();if(canSave&&!duplicate)onSave({label:label.trim(),amount:Number(amount),category,paidBy,split,date,participants:split==="appearances"?undefined:[...participants]})}}>
    ...
    {split==="appearances"&&<div className={totalAppearances?"split-note":"split-note warning"}><strong>{category} · {totalAppearances} eligible completed-game appearances</strong><span>{totalAppearances?"Each included player pays in proportion to their eligible appearances. Players excluded from a game’s split are not charged for that appearance.":"Mark games Completed and include lineup players; this expense will recalculate automatically."}</span></div>}
