...
function SettlementView({team,league,balances,games,expenses,credits,payments,players,canManage,onRecord,onDelete,exportCsv,notify}:{team:Team;league:League;balances:PlayerBalance[];games:Game[];expenses:Expense[];credits:Credit[];payments:SettlementPayment[];players:Player[];canManage:boolean;onRecord:()=>void;onDelete:(payment:SettlementPayment)=>void;exportCsv:()=>void;notify:(s:string)=>void}){
  const [detail,setDetail]=useState<number|null>(null);const [balanceFilter,setBalanceFilter]=useState<"all"|"pending">("pending");
  ...
  const transfers=settlementTransfers(balances);const shown=balances.filter(balance=>balanceFilter==="all"||Math.abs(balance.balance)>.005);
  ...
  const paidTotal=payments.reduce((sum,payment)=>sum+payment.amount,0);const remainingTotal=balances.filter(balance=>balance.balance<-.005).reduce((sum,balance)=>sum+Math.abs(balance.balance),0);
  return <><div className="view-toolbar"><div><h2>League settlement</h2><p>Every positive contribution and every shared cost is shown in the calculation.</p></div><div className="toolbar-actions">{canManage&&transfers.length>0&&<button className="primary" onClick={onRecord}>✓ Record payment</button>}<button className="ghost" onClick={shareSettlement}>↗ Share remaining</button><button className="ghost" onClick={exportCsv}>↓ Download CSV</button></div></div>
    ...
    <div className="settlement-filter"><span>Show</span><div className="balance-filter" role="group" aria-label="Filter settlement balances"><button className={balanceFilter==="pending"?"active":""} onClick={()=>setBalanceFilter("pending")}>Pending only</button><button className={balanceFilter==="all"?"active":""} onClick={()=>setBalanceFilter("all")}>All players</button></div></div>
    ...
    {transfers.length>0&&<section className="transfer-panel"><h2>Who should pay whom now</h2><p>These suggestions already account for expenses paid, credits earned, and confirmed payments.</p>{transfers.map((transfer,index)=><div key={index}><strong>{transfer.from}</strong><span>pays</span><strong>{transfer.to}</strong><b>{money.format(transfer.amount)}</b></div>)}</section>}
