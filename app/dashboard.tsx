...
export default function Dashboard({ user }: { user: { name: string; email: string } }) {
  ...
  const [modal, setModal] = useState<null | "team" | "league" | "player" | "game" | "expense" | "credit" | "payment">(null);
  ...
  return <main className={`app-shell ${sidebarCollapsed?"sidebar-collapsed":""}`}>
    ...
    {modal==="credit" && league && <CreditModal players={players} onClose={()=>setModal(null)} onSave={credit=>{updateLeague(l=>({...l,credits:[...(l.credits??[]),{...credit,id:Date.now()}]}));setModal(null);notify("Player credit added and settlement recalculated")}}/>}
...
function CreditModal({credit,players,onClose,onSave,onDelete}:{credit?:Credit;players:Player[];onClose:()=>void;onSave:(credit:Omit<Credit,"id">)=>void;onDelete?:()=>void}) {
  const localToday=()=>{const now=new Date();return `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`};
  const sortedPlayers=useMemo(()=>[...players].sort((a,b)=>a.name.localeCompare(b.name,undefined,{sensitivity:"base",numeric:true})),[players]);
  const [label,setLabel]=useState(credit?.label??""); const [amount,setAmount]=useState(credit?String(credit.amount):""); const [playerId,setPlayerId]=useState(credit?.playerId??sortedPlayers[0]?.id??0); const [date,setDate]=useState(credit?.date??localToday()); const [customPlayers,setCustomPlayers]=useState<number[]>(credit?.participants??[]);
  const participants=customPlayers;
  return <div className="modal-backdrop"><form className="modal lineup-modal" onSubmit={e=>{e.preventDefault();if(participants.length)onSave({label:label.trim(),amount:Number(amount),playerId,date,split:"custom",participants:[...participants]})}}><ModalHead eyebrow={credit?"EDIT CREDIT":"PLAYER CREDIT / WAIVER"} title={credit?"Edit player credit":"Credit a contribution"} description="Use this for umpiring or other work. The selected custom group funds the credit." close={onClose}/><div className="form-grid"><label className="wide">Reason<input autoFocus required value={label} onChange={e=>setLabel(e.target.value)} placeholder="e.g. Umpiring vs SuperKings"/></label><label>Credit amount ($)<input required min=".01" step=".01" type="number" value={amount} onChange={e=>setAmount(e.target.value)}/></label><label>Date<input required type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label>Credit to player<select required value={playerId} onChange={e=>setPlayerId(Number(e.target.value))}>{sortedPlayers.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>Who funds it?<select value="custom" disabled><option value="custom">Custom players</option></select></label></div><ParticipantPicker players={sortedPlayers} selected={customPlayers} setSelected={setCustomPlayers} title="Select the players who should fund this credit"/><div className="modal-actions">{onDelete&&<button type="button" className="danger-action" onClick={onDelete}>Delete credit</button>}<span className={participants.length?"ready":"warning"}>{participants.length} sharing</span><button type="button" className="ghost" onClick={onClose}>Cancel</button><button className="primary" disabled={!participants.length}>{credit?"Save changes":"Add credit"}</button></div></form></div>;
}
