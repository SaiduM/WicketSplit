...
function CalculatorView({players,games,expenses,credits,balances}:{players:Player[];games:Game[];expenses:Expense[];credits:Credit[];balances:PlayerBalance[]}) {
  ...
  return <>
    ...
    <section className="calculator-formula"><div><span>Completed games</span><strong>{completed.length}</strong></div><div><span>Split-eligible appearances</span><strong>{totalAppearances}</strong></div><div><span>Appearance-weighted team costs</span><strong>{money.format(weightedTotal)}</strong></div><p><b>Formula</b> Team-funded share = cost × eligible appearances ÷ total eligible appearances. Each eligible appearance has the same {money.format(costPerAppearance)} cost.</p></section>
