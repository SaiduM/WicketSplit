...
export default function Dashboard({ user }: { user: { name: string; email: string } }) {
  ...
  const invitePlayer = async (player: Player, role: "member"|"treasurer") => {
    ...
    try{
      if(player.email){
        ...
        const body=encodeURIComponent(`Hi ${player.name},\n\n${text}\n\nThis private link expires in 7 days and must be accepted using ${player.email}.`);
        window.location.assign(`mailto:${encodeURIComponent(player.email)}?subject=${subject}&body=${body}`);
        notify(`Invite prepared for ${player.email}`);
      }else if(navigator.share)await navigator.share({title:`Join ${team.name}`,text});
      else{await navigator.clipboard.writeText(text);notify("Invite link copied — share it with the player")}
    }catch(error){if((error as Error).name!=="AbortError")notify("Invitation could not be shared")}
  };
  ...
  function exportCsv() {
    ...
    const rows = [["Team",team.name],["League",league.name],["Season",league.season],[],
      ...
      ...balances.map(b => [b.name,games.filter(g=>g.status==="Completed"&&g.players.includes(b.id)).length,b.paid.toFixed(2),b.credit.toFixed(2),b.share.toFixed(2),b.balance.toFixed(2)]),
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {!team ? <EmptyState icon="♙" title="Register your first team" text="Create a team workspace, add its roster, then organize expenses by league." action="Register team" onAction={()=>setModal("team")}/> :
       !league && view!=="roster" && view!=="leagues" ? <EmptyState icon="▤" title="Create your first league" text="A team can have as many leagues and seasons as you need." action="Create league" onAction={()=>setModal("league")}/> : <>
        ...
        {view==="roster" && <RosterView team={team} canManage={isTreasurer} onInvite={setInviteTarget} onAdd={()=>setModal("player")} onEdit={setEditingPlayer}/>}
        ...
        {view==="games" && league && <GamesView games={games} players={players} canManage={isTreasurer} onAdd={()=>setModal("game")} onRoster={()=>setView("roster")} onChange={next=>updateLeague(l=>({...l,games:next}))} notify={notify}/>}
        {view==="expenses" && league && <ExpensesView expenses={expenses} credits={credits} players={players} games={games} canManage={isTreasurer} memberEmail={user.email} onAdd={()=>setModal("expense")} onCredit={()=>setModal("credit")} onRoster={()=>setView("roster")} onEdit={setEditingExpense} onEditCredit={setEditingCredit}/>}
        {view==="settlement" && league && <SettlementView team={team} league={league} balances={balances} games={games} expenses={expenses} credits={credits} players={players} exportCsv={exportCsv} notify={notify}/>}
      </>}
    </section>
    ...
    {modal==="expense" && league && <ExpenseModal memberPlayerId={memberPlayerId} existingExpenses={expenses} leagueFeeExists={expenses.some(e=>e.category==="League fee")} players={players} games={games} onClose={()=>setModal(null)} onSave={expense=>{updateLeague(l=>({...l,expenses:[...l.expenses,{...expense,id:Date.now(),submittedBy:user.email.toLowerCase()}]}));setModal(null);notify("Expense added and split recalculated")}}/>}
    {editingExpense && league && <ExpenseModal expense={editingExpense} existingExpenses={expenses} leagueFeeExists={expenses.some(e=>e.category==="League fee"&&e.id!==editingExpense.id)} players={players} games={games} onClose={()=>setEditingExpense(null)} onDelete={()=>{if(confirm(`Delete “${editingExpense.label}”? This will recalculate every balance.`)){updateLeague(l=>({...l,expenses:l.expenses.filter(e=>e.id!==editingExpense.id)}));setEditingExpense(null);notify("Expense deleted and balances recalculated")}}} onSave={expense=>{updateLeague(l=>({...l,expenses:l.expenses.map(e=>e.id===editingExpense.id?{...expense,id:e.id}:e)}));setEditingExpense(null);notify("Expense updated and split recalculated")}}/>}
    ...
    {inviteTarget&&team&&<InviteAccessModal player={inviteTarget} teamName={team.name} onClose={()=>setInviteTarget(null)} onInvite={async role=>{await invitePlayer(inviteTarget,role);setInviteTarget(null)}}/>}
...
function GamesView({games,players,canManage,onAdd,onRoster,onChange,notify}:{games:Game[];players:Player[];canManage:boolean;onAdd:()=>void;onRoster:()=>void;onChange:(g:Game[])=>void;notify:(s:string)=>void}) {
  ...
  return <>{games.length?<><div className="view-toolbar"><div><h2>League games</h2><p>{games.length} {games.length===1?"game":"games"} recorded in this league.</p></div>{canManage&&<button className="primary" onClick={onAdd}>＋ Add game</button>}</div><div className="game-grid">{games.map((g,i)=><article className="game-card" key={g.id}><div><span className={g.status==="Completed"?"status complete":"status"}>{g.status}</span><span>GAME {i+1}</span></div><div className="game-date"><strong>{new Date(g.date+"T12:00").toLocaleDateString("en-US",{day:"2-digit"})}</strong><span>{new Date(g.date+"T12:00").toLocaleDateString("en-US",{month:"short"}).toUpperCase()}</span></div><h3>vs {g.opponent}</h3><p>{g.venue||"Venue not specified"}</p><div className="selected-count"><strong>{g.players.length}</strong> selected <span className={g.players.length>=11?"valid":"invalid"}>{g.players.length>=11?"Ready":"Need more"}</span></div>{canManage&&<button className="card-action" onClick={()=>setEditing(g.id)}>Edit game & Playing {g.players.length||"XI"} →</button>}</article>)}</div></>:<EmptyState icon="◉" title="No games in this league" text="The treasurer has not recorded a game yet." action={canManage?"Add first game":undefined} onAction={canManage?onAdd:undefined}/>}
...
function ExpensesView({expenses,credits,players,games,canManage,memberEmail,onAdd,onCredit,onRoster,onEdit,onEditCredit}:{expenses:Expense[];credits:Credit[];players:Player[];games:Game[];canManage:boolean;memberEmail:string;onAdd:()=>void;onCredit:()=>void;onRoster:()=>void;onEdit:(expense:Expense)=>void;onEditCredit:(credit:Credit)=>void}) {
...
function SettlementView({team,league,balances,games,expenses,credits,players,exportCsv,notify}:{team:Team;league:League;balances:(Player&{paid:number;credit:number;share:number;balance:number})[];games:Game[];expenses:Expense[];credits:Credit[];players:Player[];exportCsv:()=>void;notify:(s:string)=>void}) {
  ...
  return <><div className="view-toolbar"><div><h2>League settlement</h2><p>Tap any player to see exactly how their share was calculated.</p></div><div className="toolbar-actions"><button className="ghost" onClick={shareSettlement}>↗ Share summary</button><button className="primary" onClick={exportCsv}>↓ Download CSV</button></div></div><div className="table-panel"><table><thead><tr><th>Player</th><th>Games</th><th>Cash paid</th><th>Credits</th><th>Fair share</th><th>Balance</th><th></th></tr></thead><tbody>{balances.map(b=><tr key={b.id}><td><div className="player-cell"><span className="avatar" style={{background:b.color}}>{b.initials}</span><strong>{b.name}</strong></div></td><td>{games.filter(g=>g.status==="Completed"&&g.players.includes(b.id)).length}</td><td>{money.format(b.paid)}</td><td className="positive">{money.format(b.credit)}</td><td>{money.format(b.share)}</td><td><strong className={b.balance>=0?"positive":"negative"}>{b.balance>=0?"+":"−"}{money.format(Math.abs(b.balance))}</strong></td><td><button className="table-edit" onClick={()=>setDetail(b.id)}>Details</button></td></tr>)}</tbody></table></div>{transfers.length>0&&<section className="transfer-panel"><h2>Suggested payments</h2><p>Use these transfers to settle the league with the fewest practical payments.</p>{transfers.map((t,i)=><div key={i}><strong>{t.from}</strong><span>pays</span><strong>{t.to}</strong><b>{money.format(t.amount)}</b></div>)}</section>}{detail!==null&&<PlayerBreakdown player={balances.find(b=>b.id===detail)!} expenses={expenses} credits={credits} games={games} players={players} onClose={()=>setDetail(null)}/>}</>;
}

function PlayerBreakdown({player,expenses,credits,games,players,onClose}:{player:Player&{paid:number;credit:number;share:number;balance:number};expenses:Expense[];credits:Credit[];games:Game[];players:Player[];onClose:()=>void}) {
...
function ExpenseModal({expense,memberPlayerId,existingExpenses,leagueFeeExists,players,games,onClose,onSave,onDelete}:{expense?:Expense;memberPlayerId?:number;existingExpenses:Expense[];leagueFeeExists:boolean;players:Player[];games:Game[];onClose:()=>void;onSave:(e:Omit<Expense,"id">)=>void;onDelete?:()=>void}) {
