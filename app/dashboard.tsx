...
export default function Dashboard({ user }: { user: { name: string; email: string } }) {
  ...
  const deleteTeam = async () => {
    if(!team||!confirm(`Permanently delete ${team.name}? This removes its roster, leagues, games, expenses, credits, memberships, and invitations for everyone.`))return;
...
function GamesView({games,expenses,credits,players,canManage,onAdd,onRoster,onChange,notify}:{games:Game[];expenses:Expense[];credits:Credit[];players:Player[];canManage:boolean;onAdd:()=>void;onRoster:()=>void;onChange:(g:Game[])=>void;notify:(s:string)=>void}) {
