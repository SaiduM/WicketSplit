...
export async function POST(request: Request) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO team_invites (token_hash, team_id, player_id, created_by, expires_at) VALUES (?, ?, ?, ?, ?)").bind(tokenHash,teamId,playerId,email,expiresAt),
  ]);
  return Response.json({ url: `${new URL(request.url).origin}/api/invites/accept?token=${token}`, expiresAt });
}
