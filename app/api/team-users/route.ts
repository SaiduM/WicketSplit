...
async function requireTreasurer(teamId:number,email:string){
  return (await env.DB.prepare("SELECT role FROM team_memberships WHERE team_id = ? AND email = ?").bind(teamId,email).first<{role:string}>())?.role==="treasurer";
}
...
export async function GET(request:Request){
  ...
  if(!await requireTreasurer(teamId,email))return Response.json({error:"Only a team treasurer can view team users"},{status:403});
  ...
  const [owner,memberships,invites,team,lastUpdate]=await Promise.all([
    ownerEmail(teamId),
  ...
  return Response.json({
    users:memberships.results.map(row=>({
      ...
      role:row.role==="treasurer"?(row.email===owner?"Owner":"Co-treasurer"):"Player",
...
export async function DELETE(request:Request){
  ...
  if(!await requireTreasurer(teamId,email))return Response.json({error:"Only a team treasurer can remove player sessions"},{status:403});
